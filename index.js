...
const { createQueryResolver } = require("@miniql/json");

async function main() {

    //
    // Configures JSON files to be loaded and how they relate to each other.
    //
    const jsonFilesConfig = {
        species: {
            primaryKey: "name",
            jsonFilePath: "./data/species.json",
            nested: {
                homeworld: {
                    parentKey: "homeworld",
                    from: "planet",
    ...
    // 
    // Loads JSON files and creates a MiniQL query resolver.
    //
    const queryResolver = await createQueryResolver(jsonFilesConfig);
